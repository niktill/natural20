...
const cors_url = 'https://morning-oasis-35115.herokuapp.com/';
...
export class AddCardForm extends React.Component {
  ...
  addCards = (event) => {
    if (this.state.addCardOptions.cardType) {
      ...
      let numCards =
        this.state.addCardOptions.cardType === 'monster'
          ? this.state.addCardOptions.quantity
          : 1;
      let cardsToAdd = [];

      for (let i = 0; i < numCards; i++) {
        // calulate initiative
        let initiativeModifierVal =
          event.target.initiativeModifier.value !== ''
            ? parseInt(event.target.initiativeModifier.value)
            : 0;
        let initiativeVal = this.state.addCardOptions.rollInitiative
          ? Math.floor(Math.random() * 20) + 1 + initiativeModifierVal
          : event.target.initiative.value;
        // create new card
        let newCard = {
          id: shortid.generate(),
  ...
  monsterSearch = (event, data) => {
    ...
    if (searchTerm) {
      ...
      let formatFilterResults = filteredResults.map((monster) => {
        monster.title = monster.name;
        return monster;
      });
      this.setState({
        searchResults: formatFilterResults,
        searchIsLoading: false,
      });
    }
  };
  ...
  handleMonsterSelect = (event, data) => {
    if (data.result) {
      this.setState({ searchIsLoading: true });
      fetch(cors_url + 'https://dnd5eapi.co' + data.result.url)
        .then((res) => {
          if (!res.ok) {
        ...
        .then((data) => {
          ...
          addCardOptionsCopy.initiativeModifier = this.abilityModifier(
            data.dexterity
          );
          this.setState({
            addCardOptions: addCardOptionsCopy,
            searchIsLoading: false,
          });
        })
        .catch((err) => {
  ...
  componentDidMount() {
    fetch(cors_url + 'https://dnd5eapi.co/api/monsters')
      .then((res) => {
        console.log(res);
        if (!res.ok) {
  ...
  render() {
    return (
      <Form id='add-card-form' onSubmit={this.addCards}>
        <Message
          className='CardTypeSelectedErrorMessage'
          size='mini'
          negative
          compact
        ...
        <Form.Group>
          <Form.Select
            ...
            options={[
              { key: 'p', text: 'Player', value: 'player' },
              { key: 'm', text: 'NPC / Monster', value: 'monster' },
            ]}
          />
          <Form.Input
            required={true}
            ...
            placeholder='Name'
            name='name'
            value={this.state.addCardOptions.name}
            onChange={this.changeAddCardOptions}
          />
        </Form.Group>
        <Form.Group>
          <div
            className={
              'field' +
              (this.state.addCardOptions.cardType !== 'monster' ||
              this.state.errorInLoadingMonsters ||
              this.state.waitingForMonsterList
                ? ' disabled'
                : '')
            }
          >
            ...
            <Search
              size='small'
              onSearchChange={this.monsterSearch}
              onFocus={this.monsterSearch}
              onResultSelect={this.handleMonsterSelect}
              loading={
                this.state.searchIsLoading || this.state.waitingForMonsterList
              }
        ...
        <Form.Group inline={true}>
          <Popup
            ...
            size='small'
          />
          <Form.Button type='submit' id='add-card-button' positive={true}>
            {' '}
            Add to Battle
          </Form.Button>
        </Form.Group>
